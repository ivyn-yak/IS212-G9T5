...
class TestStaffPending(TestApp):
    def test_staff_pending(self):
        response = self.client.get("/api/pending/140008", content_type='application/json')
    ...
    def test_staff_no_pending(self):
        response = self.client.get("/api/pending/140001", content_type='application/json')
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["data"], [])

    def test_invalid_staff(self):
        response = self.client.get("/api/pending/0", content_type='application/json')
